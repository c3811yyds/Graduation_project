...
8. Redis 驱动的性能与防刷优化（缓存 / 限流 / 验证码）：对课程列表、个人总览、管理员概览与总览接入 Redis 缓存，并为登录失败、验证码请求、AI 对话接入 Redis 限流；验证码值与冷却时间优先走 Redis，数据库保留兼容兜底，使系统在性能、稳定性与抗滥用能力上进一步完善。
...
7. Redis 缓存与限流一体化优化
  系统将 Redis 从单纯的“附加组件”落到了真实业务链路中：一方面对课程列表、个人总览、管理员概览与总览等热点读接口做缓存和失效控制，降低数据库压力；另一方面把验证码、登录失败统计、验证码请求频率和 AI 请求频率纳入 Redis 限流与冷却控制，使性能优化与安全治理形成一体化方案。
